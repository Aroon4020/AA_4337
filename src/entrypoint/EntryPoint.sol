...
contract EntryPoint is IEntryPoint, StakeManager {
    ...
    unchecked {
        for (uint256 i = 0; i < opslen; i++) {
            UserOpInfo memory opInfo = opInfos[i];//empty
    ...
    unchecked {
        ...
        if (callData.length > 0) {
            bool success = Exec.call(mUserOp.sender, 0, callData, callGasLimit);//to,value,data,gas =>call to wallet contract
            if (!success) {
    ...
    */
    ...
    unchecked {
        ...
        //validatesignature, update Nonce, pay gas to entrypoint if required
    ...
    private returns (uint256 validationData, uint256 paymasterValidationData) {
        ...
        (uint256 requiredPreFund) = _getRequiredPrefund(mUserOp);//min amount of native bal required
        (gasUsedByValidateAccountPrepayment, validationData) = _validateAccountPrepayment(opIndex, userOp, outOpInfo, requiredPreFund);//call to wallet for sig verification etc
