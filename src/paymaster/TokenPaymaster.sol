...
contract TokenPaymaster is ITokenPaymaster, Ownable {
    ...
    constructor(IEntryPoint _entryPoint, address _owner, address _walletFactory, address _treasury, address _router, address _WETH9) {
        require(address(_entryPoint) != address(0), "invalid entrypoint addr");
        require(address(_walletFactory) != address(0), "invalid factory addr");
    ...
    function setToken(
        ...
        for (uint256 i = 0; i < _token.length;) {
            ...
            address currentPriceOracle = address(supportedToken[token]);//price oracle against token
    ...
    function _postOp(
        ...
        uint256 tokenRequiredFund = ((actualGasCost + costOfPost) *
            exchangeRate) / 10 ** 18;
        IERC20(token).safeTransferFrom(sender, address(this), tokenRequiredFund); //remainig amount return to paymaster
    }
    ...
    /**
     * swap And deposit ETH to entrypoint
     * @param token to swap
     * @param amount to swap
     * @param amountOutMinimum minimum amountout after swap
     * @param fee pool fee
     */
    ...
    function getDeposit() public view returns (uint112) {
        return _IEntryPoint.getDepositInfo(address(this)).deposit;
    }

    /**
     * return current paymaster's stake on the entryPoint.
     */
    function getStake() public view returns (uint112) {
        return _IEntryPoint.getDepositInfo(address(this)).stake;
    }
    ...
    function _withdrawToken(address token, uint256 amount) private {
    ...
    // withdraw token from this contract
    function withdrawTokens(address[] calldata token, uint256[] calldata amount) external onlyOwner {
        require(token.length == amount.length, "length mismatch");
        for (uint256 i = 0; i < token.length;) {
            _withdrawToken(token[i], amount[i]);
            unchecked{
                ++i;
            }
        }
